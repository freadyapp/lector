...
export default class PragmaWord extends Pragma {
  ...
  get currentWord() {
    ...
    if (!subW)
      return util.throwSoft(`Could not find current Word of ${this.key}`);
  ...
  isInTheSameLine(n) {
    return (
      this.sibling(n) != null && (this.sibling(n).top - this.top) ** 2 < 10
    );
  }
  get isFirstInLine() {
  ...
  get difficultyIndex() {
    ...
    if (!this._difficultyIndex)
      this._difficultyIndex = generateDifficultyIndex(this);
        ...
        this.currentPromise.catch((e) => {
          ...
          this.mark
            ...
            .catch((e) => {
              ...
              console.warn(
                "prevent pause event from bubbling. Chill on the keyboard bro",
                e
              );
            })
            .then(() => {
  ...
  summon(silent = false) {
    ...
    return new PinkyPromise((resolve) => {
      this.parent
        .pause()
        .catch(() => console.log("no need to pause"))
        .then(() => {
