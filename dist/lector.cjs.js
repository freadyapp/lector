...
let displayElement = (title) => {
    return pragmajs._e(`div#${title}-display.display`, 0).setData({'settingTarget': 'display'})
};

let sectionElement = (title, htmlTemp = v => `<div class='title' id='${title}-title'>${v}</div>`) =>
    pragmajs._e(`div.collapsed-section.collapsable#${title}-section`)
        .html(htmlTemp(title))
        .append(displayElement(title));

let settingTemplate = (pragma, id, title = id) =>
    pragmajs._e(`div.setting#${pragma.key}`)
        .setData({ 'setting': id })
...
function addSettingsToLector(lector){
  ...
  function createModeIcons(mode){
    let modeThingy = pragmajs._e('div.mode-icon').setId(`${mode}`).html('W');

    let pointer = pragmajs._e(`div#qwer`).append(modeThingy).html();
