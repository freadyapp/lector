(function (global, factory) {
  ...
  function onScroll(cb){
    if (!globalThis.lectorSpace.scrollChain){
      $.createChains(globalThis.lectorSpace, 'scroll');
      _onScroll((scroll, ds) => {
  ...
  function onScrollEnd(cb){
    if (!globalThis.lectorSpace.scrollEndChain){
      $.createChains(globalThis.lectorSpace, 'scrollEnd');
...
`;
  ...
  function infinityPaginator(streamer, pageTemplate, config={}){
    let inf = W("infinity paginator")
          .from(
            paginator(pageTemplate, $.objDiff(
              {
                streamer: streamer,
          ...
          .run({
            initialConfig(){
              ...
              this.fill = function(){
                ...
                let pagesToRenderBefore = $.aryDiff(pagesToRender, pagesToRenderAfter);
  ...
  const select = (conf) => _p()
      ...
      .run(function(){
        ...
        if (!options) return $.throwSoft('need to define options when creating a select template')
  ...
  function input(conf = {}) {
      return new N()
          .from($.createTemplate(conf))
          .run({
              makeChains(){
                  $.createChains(this, 'userInput');
              },
              makeInput () {
    ...
    let settings = W("settingsWrapper")
                    ...
                    .run(function(){
                      ...
                      this._setVal = function(edit){
                        this.value = $.objDiff(this.value, edit);
                      };
...
`).contain(pageComp).pragmatize();
  ...
  const Lector = (l, options=default_options) => {
    ...
    if (options.experimental &&
      ...
      let lector = Reader(j(l).parentElement, options)
