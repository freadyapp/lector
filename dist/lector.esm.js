import { Pragma, _e as _e$1, util as util$1, _p, runAsync, _thread } from 'pragmajs';
...
var full = "@charset \"utf-8\";@import url(https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@300;400;700&family=IBM+Plex+Sans:wght@300;400;700&display=swap);body{background-color:#161616}.flex,.inline-icon,.inline-icon-2{display:flex;justify-content:center;align-items:center}.inline-icon,.inline-icon-2{width:22px}.inline-icon svg,.inline-icon-2 svg{width:22px}.inline-icon-2{width:25px}.inline-icon-2 svg{width:25px}.clickable{cursor:pointer}.meta{opacity:.5}.blurred-bg{width:100vw;height:100vh;position:fixed;top:0;left:0;z-index:9999999999999999999;background:rgba(255,255,255,0.15);backdrop-filter:blur(7px);-webkit-backdrop-filter:blur(7px)}.popUp{width:400px;height:350px;background:#161616;border-radius:4px;position:absolute;top:30%;left:50%;margin-left:-200px;padding:10px}.popUp .next-btn{position:absolute;right:-60px;width:50px;height:50px;background:#161616;border-radius:50px;top:50%;margin-top:-25px;cursor:pointer}.popUp .next-btn .next-icon{text-align:center;position:absolute;top:50%;left:50%;transform:translate(-50%,-50%) rotate(180deg)}.popUp .next-btn .next-icon>svg{width:12px;height:auto}.popUp .popUpContent{width:100%;height:100%;box-sizing:border-box;transition:all ease .5s}.popUp .popUpContent .boat{width:100%;height:100%;display:flex;flex-direction:column;flex-wrap:nowrap;justify-content:space-around;align-items:center;align-content:stretch;text-align:center}.popUp .popUpContent .boat-title{font-family:'IBM Plex Sans',sans-serif;color:whitesmoke;margin:0;font-weight:400;-webkit-touch-callout:none;-webkit-user-select:none;-khtml-user-select:none;-moz-user-select:none;-ms-user-select:none;user-select:none}.popUp .popUpContent .boat .spacebar-icon>svg{width:230px}.popUp .popUpContent .boat .speed-icon>svg{width:315px}.popUp .popUpContent .boat .click-icon>svg{width:315px}.displayN{display:none !important}";
...
function lectorSettings(lector){
  ...
  const actions = {
    changeColor(hex=this.value){
      ...
      _e$1('body').findAll('[data-lector-marker-color]').forEach(e => {
    ...
    changeMode(mode=this.value){
      ...
      _e$1('body').findAll('[data-lector-marker-mode]').forEach(e => {
  ...
  let modeIcon = _p().as(_e$1(icons['mode-icon']))
  ...
  let setMode = _p('!mode')
                  .run(function(){
                    ...
                    optionTemplate: option => _p(option)
                        ...
                        .run(function(){
                          this._miniPointer = _e$1('div.mini-pointer#');
  ...
  let foveaIcon = _p().as(_e$1(icons['fovea-icon']))
                  .addClass(`setting-icon`);
  _p('monitor')
                    .as(_e$1('div.'))
  ...
  let wpmIcon = _p().as(_e$1(icons['speed-icon']))
                .css('width 25px; height 25px;')
                .addClass(`setting-icon`);

  let wpmIncreaseIcon = _p().as(_e$1(icons['increase']))
                      .addClass(`setting-wpm-adjusticon`)
                      .listenTo('click', _ => {
                        setWpm.value += 10;
                      });

  let wpmDecreaseIcon = _p().as(_e$1(icons[`decrease`]))
                        .addClass(`setting-wpm-adjusticon`)
                        .listenTo('click', _ => {
                          setWpm.value -= 10;
                        });
  ...
  let settingsIcon = _p().as(_e$1(icons['settings-icon-white']))
...
let displayElement$1 = (key) => {
    return _e$1(`div#${key}-display.display`, 0).setData({'settingTarget': 'display', 'pragmaTarget': `${key} monitor`})
};
...
class SettingInline extends Pragma {
    constructor() {
        super();
        this.init(...arguments);
    }
...
let displayElement = (key) => {
    return _e$1(`div#${key}-display.display`, 0).setData({ 'settingTarget': 'display' })
};
...
let sectionElement = ({
    ...
    _e$1(`div.collapsed-section.collapsable#${key}-section`)
        .html(htmlTemp(key, title))
        .append(displayElement(key));

let _settingTemplate = (pragma, key) =>
    _e$1(`div.setting.collapsable#${key}`)
        .setData({ 'setting': key })
...
let makeOptionElement = function(content, wrapper){
    this.as(_e$1(wrapper(content, this))).setId(util$1.rk5()).addClass('option');
...
const edibleDisplayTemplate = (pragma) => _e$1('input')
                                    .attr('type', 'text')
...
class EdibleDisplay extends Pragma {
...
//  valueSanitizer, monitorTemplate,
        ...
        if (plusElement){
            if (!this.arrows) this.arrows = _e$1("div.arrows").appendTo(this.element);
            let _plus = _e$1(typeof plusElement === 'function' ? plusElement(this) : plusElement)
                    .listenTo('mousedown', () => {
                        this[setting] += step || 1;
                    });
        ...
        if (minusElement){
            if (!this.arrows) this.arrows = _e$1("div.arrows").appendTo(this.element);
            let _minus = _e$1(typeof minusElement === 'function' ? minusElement(this) : minusElement)
                    .listenTo('mousedown', () => {
                        this[setting] -= step || 1;
                    });
    ...
    _content(wire){
        ...
        return _p().append(
                        _e$1(`div.section#${wire}-section`)
                            .append(_e$1(`div#title.`, this.displayName || wire))
                            .append(this._edible)
                        )
...
util$1.addStyles(css.slider); // add styles for the slider
...
let settingsComp = _e$1(`div.settings`);
...
function addSettingsToLector(lector){
  ...
  function createColorBlob(color){
    let colorThingy  = _e$1(`div.color-blob.`)
                  .css(`background-color ${color}`)
                  .setId(`${color}`)
                  .html("   ");

    let blob = _e$1('div#color.')
                .append(colorThingy)
                .html();
  ...
  let pageSetting = new SettingInt(lector.settings, 'page', {
                     ...
                     .run(function(){
                       ...
                       this.element.append(_e$1("div#meta.flex.meta").html("/420"));
                     })
                     .on('input', (value) => {
  ...
  let pageBar = _e$1('div.bar#page-bar')
                    .append(pageSetting);
  ...
  let zoomBar = _e$1('div.bar#zoom-bar')
                    .append(zoomSetting);
  ...
  let settingsButton = _e$1('div.inline-icon.clickable#settings-icon').html(icons['settings-icon-white']);
...
class popUp extends Pragma{ 
    ...
    render(){
        this.background = _e$1('div.blurred-bg').appendTo('body');
        ...
        this.nextBtn = _e$1('div.next-btn')
                      .html(`<div class="next-icon">${icons['back-icon']}</div>`)
                      
                      .appendTo(this.popUp);

        this.popUpContent = _e$1('div.popUpContent').appendTo(this.popUp);
