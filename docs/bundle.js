...
class Mark extends Pragma.Pragma {
  ...
  pause() {
    return new Promise((resolve, reject) => {
      ...
      if (this.currentlyMarking && this.current_anime && this.last_marked) {
        ...
        let temp = this.last_marked; //console.table(temp)
  ...
  mark(word, time = 200, fit = false, ease = "easeInOutExpo") {
    if (!(word instanceof Pragma.Pragma)) return new Promise(r => {
...
const LectorSettings = parent => {
  ...
  let freadyBridge = (0, _pragmajs.Bridge)(settings, syncedKeys, object => {// TODO add beam
  });
...
class Word extends Pragma.Pragma {
  ...
  onread() {// triggers each time a word is read
    //console.log('yyet')
  }

  onpause() {
    console.log("on pause event for: " + this.children[this.cursor].text()); //console.log('paused reading')
  }
